        ...
        CamObject.map(function (item) {
          if (item) {
            function convertDate(date) {
              ...
              if (yyyy > 0) {
                tempdate =
                  yyyy + "y:" + mm + "m:" + dd + "d:" + hh + "h:" + mi + "m"; //+ sec+'s';
              } else if (mm > 0) {
            ...
            function calculateDuration(camDate) {
              try {
                ...
                var then = moment(
                  camDate.dataValues.StartDate,
                  "YYYY-MM-DD'T'HH:mm:ss:SSSZ"
                );
                var now = moment();

                var endDate = moment(
                  camDate.dataValues.EndDate,
                  "YYYY-MM-DD'T'HH:mm:ss:SSSZ"
                );
                    ...
                    attributes: [
                      [
                        DbConn.SequelizeConn.fn(
                          "COUNT",
                          DbConn.SequelizeConn.col("CampaignId")
                        ),
                        "total_dial",
                      ],
                      [
                        DbConn.SequelizeConn.fn(
                          "COUNT",
                          DbConn.SequelizeConn.literal(
                            `case when "DialerStatus" = 'channel_answered' then "DialerStatus" end`
                          )
                        ),
                        "channel_answered",
                      ],
                      [
                        DbConn.SequelizeConn.fn(
                          "COUNT",
                          DbConn.SequelizeConn.literal(
                            `case when "DialerStatus" = 'dial_success' then "DialerStatus" end`
                          )
                        ),
                        "dial_success",
                      ],
                      [
                        DbConn.SequelizeConn.fn(
                          "COUNT",
                          DbConn.SequelizeConn.literal(
                            `case when "DialerStatus" = 'dial_failed' then "DialerStatus" end`
                          )
                        ),
                        "dial_failed",
                      ],
                  ...
                  DbConn.CampDialoutInfo.find(query)
                    .then(function (results) {
                      var rowData = {
                        ...
                        startEndDate: startEndDateObject
                          ? startEndDateObject.dataValues
                          : "N/A",
                      };
                      if (results && results.dataValues) {
                        try {
                          var total_dial =
                            results.dataValues.total_dial !== undefined
                              ? parseInt(results.dataValues.total_dial)
                              : 0;
                          rowData.total_dial = total_dial;

                          var channel_answered =
                            results.dataValues.channel_answered !== undefined
                              ? parseInt(results.dataValues.channel_answered)
                              : 0;
                          rowData.channel_answered = channel_answered;

                          var dial_success =
                            results.dataValues.dial_success !== undefined
                              ? parseInt(results.dataValues.dial_success)
                              : 0;
                          rowData.dial_success = dial_success;

                          var dial_failed =
                            results.dataValues.dial_failed !== undefined
                              ? parseInt(results.dataValues.dial_failed)
                              : 0;
                          ...
                          rowData.percentage =
                            total_dial === 0
                              ? 0
                              : (channel_answered / total_dial) * 100;
                        } catch (ex) {
...
module.exports.CampaignDispositionReport = function (req, res) {
  ...
  DbConn.CampDialoutInfo.findAll(query)
    .then(function (CamObject) {
      if (CamObject) {
        jsonString = messageFormatter.FormatMessage(
          undefined,
...
module.exports.CampaignAttemptReport = function (req, res) {
  ...
  var query = {
    attributes: [
      "DialNumber",
      [
        DbConn.SequelizeConn.fn("MAX", DbConn.SequelizeConn.col('"createdAt"')),
        "createdAt",
      ],
      [
        DbConn.SequelizeConn.fn("COUNT", DbConn.SequelizeConn.col("TryCount")),
        "TryCount",
      ],
      [
        DbConn.SequelizeConn.fn(
...
module.exports.CampaignAttemptReportCount = function (req, res) {
  ...
  var query = {
    attributes: [
      [DbConn.SequelizeConn.literal('DISTINCT "DialNumber"'), "DialNumber"],
    ],
    where: [{ CampaignId: campaignId }],
  };
