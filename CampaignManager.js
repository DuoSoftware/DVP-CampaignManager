...
function CreateCampaign(req, callback) {
    try {
        var cmp = req.body;
        campaignHandler.CreateCampaign(cmp.CampaignName, cmp.CampaignnMode, cmp.CampaignChannel, cmp.DialoutMechanism, cmp.TenantId, cmp.CompanyId, cmp.Class, cmp.Type, cmp.Category, callback)

    } catch (ex) {
        logger.error('[DVP-CampaignManager] - [%s] - [PGSQL] - CreateCampaign', req.body, err);
    }
}
    ...
    DbConn.Campaign.find({attributes: ["id"], where: [{CampaignName: campId}]}).complete(function (err, campRes) {
        if (err) {
            ...
            if (campRes != null) {
                ...
                DbConn.CampaignPhones.findAll({
                    attributes: ["Phone", "CampaignId"],
                    where: [{CampaignId: campRes.id.toString()}, {Enable: 'true'}],
                    limit: Max
                }).complete(function (errPhn, resultPhn) {
                    if (errPhn) {
...
function GetCampaign(callback) {
    ...
    try {
        ...
        DbConn.Campaign.findAll({
            attributes: ["id", "CampaignName", "Min", "Max", "StartTime", "EndTime", "LastUpdate", "ConcurrentLimit"],
            where: [{"StartTime": {lt: nowTm}}, {"EndTime": {gt: nowTm}}]
