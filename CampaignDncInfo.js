...
function CreateDncRecord(contactIds, tenantId, companyId, res) {
  ...
  DbConn.CampDncInfo.bulkCreate(contactDetails, {
    validate: false,
    ...
    .then(function (dnc) {
      ...
      logger.info(
        "[DVP-CampDncInfo.CreateDncRecord] - [PGSQL] - inserted successfully. [%s] ",
        jsonString
      );
      res.end(jsonString);
    })
    .error(function (err) {
      logger.error(
        "[DVP-CampDncInfo.CreateDncRecord] - [%s] - [PGSQL] - insertion  failed-[%s]",
        contactIds,
        err
      );
      jsonString = messageFormatter.FormatMessage(
...
function DeleteDncRecord(contactIds, tenantId, companyId, res) {
  ...
  DbConn.CampDncInfo.destroy({
    ...
    .then(function (dnc) {
      ...
      logger.info(
        "[DVP-CampDncInfo.DeleteDncRecord] - [PGSQL] - delete successfully. [%s] ",
        jsonString
      );
      res.end(jsonString);
    })
    .error(function (err) {
      logger.error(
        "[DVP-CampDncInfo.DeleteDncRecord] - [%s] - [PGSQL] - delete  failed-[%s]",
        contactId,
        err
      );
      jsonString = messageFormatter.FormatMessage(
  ...
  DbConn.CampDncInfo.findAll({
    ...
    .then(function (dncObjects) {
      if (dncObjects) {
        ...
        logger.info(
          "[DVP-CampDncInfo.GetDncList] - [%s] - [PGSQL]  - Data found  - %s-[%s]",
          tenantId,
          companyId,
          JSON.stringify(dncNumberList)
        );
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "SUCCESS",
        ...
        logger.error(
          "[DVP-CampDncInfo.GetDncList] - [PGSQL]  - No record found for %s - %s  ",
          tenantId,
          companyId
        );
        jsonString = messageFormatter.FormatMessage(
    ...
    .error(function (err) {
      logger.error(
        "[DVP-CampDncInfo.GetDncList] - [%s] - [%s] - [PGSQL]  - Error in searching.-[%s]",
        tenantId,
        companyId,
        err
      );
